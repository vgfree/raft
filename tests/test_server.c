...
void TestRaft_server_voted_for_records_who_we_voted_for(CuTest *tc)
{
    ...
    raft_set_voted_for(r, 2);
    CuAssertTrue(tc, 2 == raft_server_get_voted_for((raft_server_private_t *)r));
}
...
void TestRaft_server_idx_starts_at_1(CuTest *tc)
{
    ...
    raft_set_callbacks(r, &generic_funcs, NULL);
    CuAssertTrue(tc, 0 == raft_cache_get_entry_last_idx(r->log));
    ...
    CuAssertTrue(tc, 1 == raft_cache_get_entry_last_idx(r->log));
}
...
void TestRaft_server_voting_results_in_voting(CuTest *tc)
{
    ...
    raft_set_voted_for(r, 1);
    CuAssertTrue(tc, 1 == raft_server_get_voted_for((raft_server_private_t *)r));
    raft_set_voted_for(r, 9);
    CuAssertTrue(tc, 9 == raft_server_get_voted_for((raft_server_private_t *)r));
}
...
void TestRaft_server_add_node_makes_non_voting_node_voting(CuTest *tc)
{
    ...
    CuAssertTrue(tc, !raft_node_is_voting((raft_node_private_t *)n1));
    raft_add_node(r, NULL, 9, 0);
    CuAssertTrue(tc, raft_node_is_voting((raft_node_private_t *)n1));
...
void TestRaft_set_state(CuTest *tc)
{
    ...
    raft_server_set_state((raft_server_private_t *)r, RAFT_STATE_LEADER);
...
void TestRaft_server_append_entry_is_retrievable(CuTest *tc)
{
    ...
    raft_set_callbacks(r, &generic_funcs, NULL);
    raft_server_set_state((raft_server_private_t *)r, RAFT_STATE_CANDIDATE);
    ...
    raft_entry_t *kept = raft_cache_dup_at_idx(r->log, 1);
    ...
    CuTest *tc)
{
    ...
    raft_server_async_retain_entries_start(r, &ety);
    raft_server_async_retain_entries_start(r, &ety);
    CuAssertTrue(tc, 2 == raft_cache_get_entry_last_idx(r->log));

    /* different ID so we can be successful */
    ety.id = 2;
    raft_server_async_retain_entries_start(r, &ety);
    CuAssertTrue(tc, 3 == raft_cache_get_entry_last_idx(r->log));
}
...
void TestRaft_server_entry_is_retrieveable_using_idx(CuTest *tc)
{
    ...
    e1.data.buf = str;
    e1.data.len = 3;
    raft_server_async_retain_entries_start(r, &e1);
    ...
    e2.data.buf = str2;
    e2.data.len = 3;
    raft_server_async_retain_entries_start(r, &e2);

    CuAssertTrue(tc, NULL != (ety_appended = raft_cache_dup_at_idx(r->log, 2)));
    ...
    CuTest *tc)
{
    ...
    /* must be follower */
    raft_server_set_state((raft_server_private_t *)r, RAFT_STATE_FOLLOWER);
    raft_set_current_term(r, 1);
    raft_server_set_last_applied_idx(r, 0);
...
{
    raft_cbs_t funcs = {
        .send_appendentries = __raft_server_send_appendentries,
    };
...
void TestRaft_server_recv_entry_auto_commits_if_we_are_the_only_node(CuTest *tc)
{
    ...
    raft_add_node(r, NULL, 1, 1);
    raft_set_election_timeout(r, 1000);
    raft_server_become_leader((raft_server_private_t *)r);
    CuAssertTrue(tc, 0 == raft_get_commit_idx(r));
...
void TestRaft_server_recv_entry_fails_if_there_is_already_a_voting_change(CuTest *tc)
{
    ...
    /* receive entry */
    msg_entry_response_t cr;
    CuAssertTrue(tc, 0 == raft_retain_entries(r, &ety, &cr));
    CuAssertTrue(tc, 1 == raft_cache_count(((raft_server_private_t *)r)->log));

    ety.id = 2;
    CuAssertTrue(tc, RAFT_ERR_ONE_VOTING_CHANGE_ONLY == raft_retain_entries(r, &ety, &cr));
    CuAssertTrue(tc, 1 == raft_get_commit_idx(r));
}
    ...
    )
{
    ...
    raft_add_node(r, NULL, 2, 0);
    raft_set_current_term(r, 1);
    CuAssertTrue(tc, 0 == raft_server_get_nvotes_for_me((raft_server_private_t *)r));
    ...
    )
{
    ...
    raft_server_become_candidate((raft_server_private_t *)r);
    CuAssertIntEquals(tc, 2, raft_get_current_term(r));
    CuAssertTrue(tc, 1 == raft_server_get_nvotes_for_me((raft_server_private_t *)r));
    ...
    int e = raft_recv_requestvote_response(r, raft_get_node(r, 2), &rvr);
    CuAssertIntEquals(tc, 0, e);
    CuAssertTrue(tc, 2 == raft_server_get_nvotes_for_me((raft_server_private_t *)r));
}
    ...
    )
{
    ...
    raft_server_become_candidate((raft_server_private_t *)r);
    raft_set_current_term(r, 1);
    CuAssertIntEquals(tc, 1, raft_server_get_voted_for((raft_server_private_t *)r));
    ...
    CuAssertIntEquals(tc, 2, raft_get_current_term(r));
    CuAssertIntEquals(tc, 2, raft_server_get_voted_for((raft_server_private_t *)r));
}
    ...
    )
{
    ...
    CuAssertIntEquals(tc, 1, raft_is_follower(r));
    CuAssertIntEquals(tc, 2, raft_get_current_term(r));
    CuAssertIntEquals(tc, 3, raft_server_get_voted_for((raft_server_private_t *)r));
}
...
void TestRaft_follower_becomes_follower_is_follower(CuTest *tc)
{
    ...
    raft_server_become_follower((raft_server_private_t *)r);
    ...
    CuTest *tc)
{
    ...
    /*  log size s */
    CuAssertTrue(tc, 0 == raft_cache_count(((raft_server_private_t *)r)->log));
...
void TestRaft_follower_recv_appendentries_increases_log(CuTest *tc)
{
    ...
    CuAssertTrue(tc, 1 == aer.success);
    CuAssertTrue(tc, 1 == raft_cache_count(((raft_server_private_t *)r)->log));
    raft_entry_t *log = raft_cache_dup_at_idx(r->log, 1);
    ...
    char            **strs)
{
    ...
    ety.term = 1;
    raft_server_async_retain_entries_start(r, &ety);
    CuAssertTrue(tc, 2 == raft_cache_count(((raft_server_private_t *)r)->log));
    CuAssertTrue(tc, NULL != (ety_appended = raft_cache_dup_at_idx(r->log, 2)));
    CuAssertTrue(tc, !strncmp(ety_appended->data.buf, str2, 3));
    ...
    ety.term = 1;
    raft_server_async_retain_entries_start(r, &ety);
    CuAssertTrue(tc, 3 == raft_cache_count(((raft_server_private_t *)r)->log));
    CuAssertTrue(tc, NULL != (ety_appended = raft_cache_dup_at_idx(r->log, 3)));
    ...
    CuTest *tc)
{
    ...
    CuAssertTrue(tc, 2 == raft_cache_count(((raft_server_private_t *)r)->log));
    /* str1 is still there */
    CuAssertTrue(tc, NULL != (ety_appended = raft_cache_dup_at_idx(r->log, 1)));
    ...
    CuTest *tc)
{
    ...
    CuAssertIntEquals(tc, 3, raft_cache_count(((raft_server_private_t *)r)->log));
    ...
    raft_recv_appendentries(r, raft_get_node(r, 2), &ae, &aer);
    CuAssertTrue(tc, 1 == aer.success);
    CuAssertIntEquals(tc, 2, raft_cache_count(((raft_server_private_t *)r)->log));
    CuAssertTrue(tc, NULL != (ety_appended = raft_cache_dup_at_idx(r->log, 1)));
    CuAssertTrue(tc, !strncmp(ety_appended->data.buf, strs[0], 3));
}
    ...
    CuTest *tc)
{
    ...
    memset(&aer, 0, sizeof(aer));
    raft_recv_appendentries(r, raft_get_node(r, 2), &ae, &aer);
    /* still successful even when no raft_server_async_retain_entries_start() happened! */
    CuAssertTrue(tc, 1 == aer.success);
    CuAssertIntEquals(tc, 1, raft_cache_count(((raft_server_private_t *)r)->log));
    ...
    CuTest *tc)
{
    ...
    ety.term = 1;
    raft_server_async_retain_entries_start(r, &ety);
    CuAssertIntEquals(tc, 2, raft_cache_get_entry_last_idx(r->log));
    ...
    CuAssertIntEquals(tc, 2, raft_cache_get_entry_last_idx(r->log));
    raft_entry_t *tmp = raft_cache_dup_at_idx(r->log, 2);
    ...
    CuAssertIntEquals(tc, 2, raft_cache_get_entry_last_idx(r->log));
    tmp = raft_cache_dup_at_idx(r->log, 2);
    CuAssertTrue(tc, NULL != tmp);
    ...
    CuAssertIntEquals(tc, 3, raft_cache_get_entry_last_idx(r->log));
}
    ...
    CuTest *tc)
{
    ...
    CuAssertTrue(tc, 1 == aer.success);
    CuAssertIntEquals(tc, 5, raft_cache_get_entry_last_idx(r->log));
}
    ...
    CuTest *tc)
{
    ...
    CuAssertTrue(tc, 1 == aer.success);
    CuAssertIntEquals(tc, 6, raft_cache_get_entry_last_idx(r->log));
    CuAssertIntEquals(tc, 4, raft_get_commit_idx(r));
...
void TestRaft_follower_becoming_candidate_votes_for_self(CuTest *tc)
{
    ...
    CuAssertTrue(tc, -1 == raft_server_get_voted_for((raft_server_private_t *)r));
    raft_server_become_candidate((raft_server_private_t *)r);
    CuAssertTrue(tc, raft_get_my_nodeid(r) == raft_server_get_voted_for((raft_server_private_t *)r));
    CuAssertTrue(tc, 1 == raft_server_get_nvotes_for_me((raft_server_private_t *)r));
}
...
void TestRaft_candidate_requestvote_includes_logidx(CuTest *tc)
{
    ...
    ety.term = 3;
    raft_server_async_retain_entries_start(r, &ety);
    raft_server_send_requestvote(r, raft_get_node(r, 2));
...
void TestRaft_non_leader_recv_entry_msg_fails(CuTest *tc)
{
    ...
    /* receive entry */
    int e = raft_retain_entries(r, &ety, &cr);
    ...
    CuTest *tc)
{
    ...
    raft_node_set_next_idx((raft_node_private_t *)p, 4);

    /* receive appendentries messages */
    raft_server_send_appendentries(r, p);
    msg_appendentries_t *ae = sender_poll_msg_data(sender);
    CuAssertTrue(tc, NULL != ae);
}
    ...
    CuTest *tc)
{
    ...
    /* receive appendentries messages */
    raft_server_send_appendentries(r, raft_get_node(r, 2));
    msg_appendentries_t *ae = sender_poll_msg_data(sender);
    CuAssertTrue(tc, NULL != ae);
    ...
    CuTest *tc)
{
    ...
    ety.data.buf = (unsigned char *)"aaa";
    raft_server_async_retain_entries_start(r, &ety);
    raft_node_set_next_idx((raft_node_private_t *)n, 1);
    raft_server_send_appendentries(r, raft_get_node(r, 2));
    ae = sender_poll_msg_data(sender);
    CuAssertTrue(tc, NULL != ae);
    CuAssertTrue(tc, ae->prev_log_idx == 0);
    ...
    /* set next_idx */
    /* receive appendentries messages */
    raft_node_set_next_idx((raft_node_private_t *)n, 2);
    raft_server_send_appendentries(r, raft_get_node(r, 2));
    ae = sender_poll_msg_data(sender);
    CuAssertTrue(tc, NULL != ae);
    ...
    CuTest *tc)
{
    ...
    ety.data.buf = (unsigned char *)"aaa";
    raft_server_async_retain_entries_start(r, &ety);
    raft_server_send_appendentries(r, n);
    ae = sender_poll_msg_data(sender);
    CuAssertTrue(tc, NULL != ae);
    CuAssertTrue(tc, ae->prev_log_idx == 0);
}
    ...
    CuTest *tc)
{
    ...
    raft_server_send_appendentries(r, raft_get_node(r, 2));
    raft_server_send_appendentries(r, raft_get_node(r, 3));
    ...
    raft_periodic(r, 1);
    CuAssertIntEquals(tc, 2, raft_server_get_last_applied_idx(r));
}
    ...
    CuTest *tc)
{
    ...
    raft_server_send_appendentries(r, raft_get_node(r, 5));
    aer.term = 1;
    aer.success = 1;
    aer.current_idx = 2;
    aer.first_idx = 1;
    ...
    CuTest *tc)
{
    ...
    CuAssertIntEquals(tc, 0, raft_get_commit_idx(r));
    raft_periodic(r, 1);
    CuAssertIntEquals(tc, 0, raft_server_get_last_applied_idx(r));
    ...
    CuAssertIntEquals(tc, 3, raft_get_commit_idx(r));
    raft_periodic(r, 1);
    CuAssertIntEquals(tc, 3, raft_server_get_last_applied_idx(r));
}
...
void TestRaft_leader_recv_entry_does_not_send_new_appendentries_to_slow_nodes(CuTest *tc)
{
    ...
    /* make the node slow */
    raft_node_set_next_idx((raft_node_private_t *)raft_get_node(r, 2), 1);
