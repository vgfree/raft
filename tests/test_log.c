...
void TestLog_new_is_empty(CuTest *tc)
{
    ...
    l = raft_cache_make(INITIAL_CAPACITY);
    CuAssertTrue(tc, 0 == raft_cache_count(l));
}
...
void TestLog_append_is_not_empty(CuTest *tc)
{
    ...
    CuAssertIntEquals(tc, 0, raft_cache_push_alone_entry(l, &e));
...
void TestLog_get_at_idx(CuTest *tc)
{
    ...
    e1.id = 1;
    CuAssertIntEquals(tc, 0, raft_cache_push_alone_entry(l, &e1));
    e2.id = 2;
    CuAssertIntEquals(tc, 0, raft_cache_push_alone_entry(l, &e2));
    e3.id = 3;
    CuAssertIntEquals(tc, 0, raft_cache_push_alone_entry(l, &e3));
    CuAssertIntEquals(tc, 3, raft_cache_count(l));
    CuAssertIntEquals(tc, e1.id, raft_cache_dup_at_idx(l, 1)->id);
    CuAssertIntEquals(tc, e2.id, raft_cache_dup_at_idx(l, 2)->id);
    CuAssertIntEquals(tc, e3.id, raft_cache_dup_at_idx(l, 3)->id);
}
...
void TestLog_get_at_idx_returns_null_where_out_of_bounds(CuTest *tc)
{
    ...
    void *r = raft_new();
    // log_set_callbacks(l, NULL, r);
    CuAssertTrue(tc, NULL == raft_cache_dup_at_idx(l, 0));
    CuAssertTrue(tc, NULL == raft_cache_dup_at_idx(l, 1));

    e1.id = 1;
    CuAssertIntEquals(tc, 0, raft_cache_push_alone_entry(l, &e1));
    CuAssertTrue(tc, NULL == raft_cache_dup_at_idx(l, 2));
}
...
void TestLog_delete(CuTest *tc)
{
    ...
    CuAssertIntEquals(tc, 0, raft_cache_push_alone_entry(l, &e3));
    CuAssertIntEquals(tc, 3, raft_cache_count(l));
    CuAssertIntEquals(tc, 3, raft_cache_get_entry_last_idx(l));

    raft_server_del_entries_after_from_idx(l, 3);
    CuAssertIntEquals(tc, 2, raft_cache_count(l));
    CuAssertIntEquals(tc, e3.id, ((raft_entry_t *)llqueue_poll(queue))->id);
    CuAssertIntEquals(tc, 2, raft_cache_count(l));
    CuAssertTrue(tc, NULL == raft_cache_dup_at_idx(l, 3));

    raft_server_del_entries_after_from_idx(l, 2);
    CuAssertIntEquals(tc, 1, raft_cache_count(l));
    CuAssertTrue(tc, NULL == raft_cache_dup_at_idx(l, 2));

    raft_server_del_entries_after_from_idx(l, 1);
...
raft_entry_t *log_peektail(raft_cache_private_t *me_)
{
    ...
    if (0 == me->tail) {
        return me->addr[me->size - 1];
    } else {
        return me->addr[me->tail - 1];
    }
}
...
void TestLog_delete_handles_log_pop_failure(CuTest *tc)
{
    ...
    CuAssertIntEquals(tc, -1, raft_server_del_entries_after_from_idx(l, 3));
    CuAssertIntEquals(tc, 3, raft_cache_count(l));
    CuAssertIntEquals(tc, 3, raft_cache_count(l));
...
void TestLog_delete_fails_for_idx_zero(CuTest *tc)
{
    ...
    CuAssertIntEquals(tc, 0, raft_cache_push_alone_entry(l, &e2));
    CuAssertIntEquals(tc, 0, raft_cache_push_alone_entry(l, &e3));
    CuAssertIntEquals(tc, 0, raft_cache_push_alone_entry(l, &e4));
    CuAssertIntEquals(tc, raft_server_del_entries_after_from_idx(l, 0), -1);
}
...
void TestLog_poll(CuTest *tc)
{
    ...
    e1.id = 1;
    CuAssertIntEquals(tc, 0, raft_cache_push_alone_entry(l, &e1));
    CuAssertIntEquals(tc, 1, raft_cache_get_entry_last_idx(l));
    ...
    /* remove 1st */
    ety = NULL;
    CuAssertIntEquals(tc, raft_cache_pop_head_entry(l, (void *)&ety), 0);
    CuAssertTrue(tc, NULL != ety);
    CuAssertIntEquals(tc, 2, raft_cache_count(l));
    CuAssertIntEquals(tc, ety->id, 1);
    CuAssertIntEquals(tc, 1, raft_cache_get_entry_head_idx(l));
    CuAssertTrue(tc, NULL == raft_cache_dup_at_idx(l, 1));
    CuAssertTrue(tc, NULL != raft_cache_dup_at_idx(l, 2));
    CuAssertTrue(tc, NULL != raft_cache_dup_at_idx(l, 3));
    CuAssertIntEquals(tc, 3, raft_cache_get_entry_last_idx(l));
...
void T_estlog_cant_append_duplicates(CuTest *tc)
{
    ...
    void *r = raft_new();
    // log_set_callbacks(l, NULL, r);
    CuAssertTrue(tc, 1 == raft_cache_push_alone_entry(l, &e));
    CuAssertTrue(tc, 1 == raft_cache_count(l));
}
...
void TestLog_delete_after_polling(CuTest *tc)
{
    ...
    /* poll */
    CuAssertIntEquals(tc, raft_server_del_entries_after_from_idx(l, 1), 0);
    CuAssertIntEquals(tc, 0, raft_cache_count(l));
}
...
void TestLog_get_from_idx_with_base_off_by_one(CuTest *tc)
{
    ...
    CuAssertPtrEquals(tc, raft_server_get_series_same_type_entries_from_idx(l, 1), NULL);

    /* now get the correct index */
    raft_batch_t *bat = raft_server_get_series_same_type_entries_from_idx(l, 2);
    CuAssertPtrNotNull(tc, bat);
    CuAssertIntEquals(tc, bat->n_entries, 1);
